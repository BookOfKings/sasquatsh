...
import { TOURNAMENT_EVENT_TYPES } from '@/types/yugioh'
...
// Show OTS Points for official tournament events
const showOtsPoints = computed(() =>
  props.config.awardsOtsPoints && TOURNAMENT_EVENT_TYPES.includes(props.config.eventType)
)
...
const currencySymbol = computed(() => {
  ...
  return symbols[props.config.entryFeeCurrency] || props.config.entryFeeCurrency
})
</script>
      ...
      <div class="flex flex-wrap gap-6">
        ...
        <div v-if="config.entryFee !== null && config.entryFee !== undefined && config.entryFee > 0" class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-gray-900">{{ currencySymbol }}{{ config.entryFee.toFixed(2) }}</p>
          </div>
        </div>
        ...
        <!-- OTS Points -->
        <div v-if="showOtsPoints" class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm text-gray-500">Official Event</p>
            <p class="font-semibold text-gray-900">OTS Points</p>
          </div>
        </div>
      </div>

      <!-- Prize Structure Details -->
      <div v-if="config.hasPrizes && config.prizeStructure" class="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
        <p class="text-sm font-medium text-blue-800 mb-1">Prize Structure</p>
        <p class="text-sm text-blue-700 whitespace-pre-wrap">{{ config.prizeStructure }}</p>
      </div>

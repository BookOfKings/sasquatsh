...
import { POKEMON_FORMATS, LIMITED_EVENT_TYPES } from '@/types/pokemon'

const props = defineProps<{
  config: PokemonEventConfig
}>()

// Is limited event (prerelease/draft)?
...
const format = computed(() =>
  POKEMON_FORMATS.find(f => f.id === props.config.formatId)
)

// Deck size based on format or limited event
const deckSize = computed(() => {
  if (isLimited.value) return '40+ cards'
  return format.value ? `${format.value.minDeckSize} cards` : '60 cards'
})
...
<template>
  <div v-if="hasRules" class="card">
    ...
    <div class="p-4 space-y-4">
      ...
      <div v-if="format || isLimited" class="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
        <p class="text-sm font-medium text-yellow-900 mb-2">
          {{ isLimited ? 'Limited Event' : format?.name }} Rules
        </p>
        <div class="flex flex-wrap gap-3 text-sm">
          <span class="text-yellow-700">
            <span class="font-medium">Deck:</span> {{ deckSize }}
          </span>
          <span class="text-yellow-700">
            <span class="font-medium">Copies:</span> 4x max (except Basic Energy)
          </span>
          <span v-if="isLimited" class="text-yellow-700">
            Built on-site from provided product
          </span>
        </div>
      </div>

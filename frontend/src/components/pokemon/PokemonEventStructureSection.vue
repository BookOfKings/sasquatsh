...
import {
  POKEMON_EVENT_TYPE_DESCRIPTIONS,
  TOURNAMENT_EVENT_TYPES,
  LIMITED_EVENT_TYPES,
  LIMITED_EVENT_GUIDANCE,
  DEFAULT_MAX_PLAYERS,
} from '@/types/pokemon'
...
// Event type categories with icons and descriptions
const eventTypeCategories = [
  {
    label: 'Casual',
    types: [
      { value: 'casual' as PokemonEventType, label: 'Casual Play', icon: 'M2,21H20V19H2M20,8H18V5H20M20,3H4V13A4,4 0 0,0 8,17H14A4,4 0 0,0 18,13V10H20A2,2 0 0,0 22,8V5C22,3.89 21.1,3 20,3Z' },
      { value: 'league' as PokemonEventType, label: 'League Play', icon: 'M12,8L10.67,8.09C9.81,7.07 7.4,4.5 5,4.5C5,4.5 3.03,7.46 4.96,11.41C4.41,12.24 4.07,12.67 4,13.66L2.07,13.95L2.28,14.93L4.04,14.67L4.18,15.38L2.61,16.32L3.08,17.21L4.53,16.32C5.68,18.76 8.59,20 12,20C15.41,20 18.32,18.76 19.47,16.32L20.92,17.21L21.39,16.32L19.82,15.38L19.96,14.67L21.72,14.93L21.93,13.95L20,13.66C19.93,12.67 19.59,12.24 19.04,11.41C20.97,7.46 19,4.5 19,4.5C16.6,4.5 14.19,7.07 13.33,8.09L12,8Z' },
    ]
  },
  {
    label: 'Competitive',
    types: [
      { value: 'league_challenge' as PokemonEventType, label: 'League Challenge', icon: 'M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4M11,17V16H9V14H13V13H10A1,1 0 0,1 9,12V9A1,1 0 0,1 10,8H11V7H13V8H15V10H11V11H14A1,1 0 0,1 15,12V15A1,1 0 0,1 14,16H13V17H11Z' },
      { value: 'league_cup' as PokemonEventType, label: 'League Cup', icon: 'M5,16L3,5L8.5,12L12,5L15.5,12L21,5L19,16H5M19,19A1,1 0 0,1 18,20H6A1,1 0 0,1 5,19V18H19V19Z' },
      { value: 'regional' as PokemonEventType, label: 'Regional', icon: 'M12,3L2,12H5V20H19V12H22L12,3M12,8.75A2.25,2.25 0 0,1 14.25,11A2.25,2.25 0 0,1 12,13.25A2.25,2.25 0 0,1 9.75,11A2.25,2.25 0 0,1 12,8.75Z' },
    ]
  },
  {
    label: 'Limited',
    types: [
      { value: 'prerelease' as PokemonEventType, label: 'Prerelease', icon: 'M5,3H19A2,2 0 0,1 21,5V19A2,2 0 0,1 19,21H5A2,2 0 0,1 3,19V5A2,2 0 0,1 5,3M7,5V9H9V5H7M11,5V11H13V5H11M15,5V7H17V5H15M7,11V13H9V11H7M11,13V15H13V13H11M15,9V13H17V9H15M7,15V19H9V15H7M11,17V19H13V17H11M15,15V19H17V15H15Z' },
      { value: 'draft' as PokemonEventType, label: 'Draft', icon: 'M19,3H14.82C14.25,1.44 12.53,0.64 11,1.2C10.14,1.5 9.5,2.16 9.18,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3M12,3A1,1 0 0,1 13,4A1,1 0 0,1 12,5A1,1 0 0,1 11,4A1,1 0 0,1 12,3' },
    ]
  }
]
...
// Only show tournament settings for actual tournament events
// Draft/Prerelease can optionally show if you want tournament-style play
const showTournamentSettings = computed(() => isTournament.value)

// Get event type description
const selectedTypeDescription = computed(() =>
  POKEMON_EVENT_TYPE_DESCRIPTIONS[props.eventType] || ''
)

// Get limited event guidance
const limitedGuidance = computed(() => {
  if (props.eventType === 'prerelease') return LIMITED_EVENT_GUIDANCE.prerelease
  if (props.eventType === 'draft') return LIMITED_EVENT_GUIDANCE.draft
  return null
})
...
function handleEventTypeChange(newType: PokemonEventType) {
  ...
  if (TOURNAMENT_EVENT_TYPES.includes(newType)) {
    // Competitive tournament: Swiss, Bo3, 50 min
  ...
  } else if (LIMITED_EVENT_TYPES.includes(newType)) {
    // Limited events: Swiss rounds, but simplified
    emit('update:tournamentStyle', 'swiss')
    emit('update:bestOf', 1) // Bo1 is common for prerelease
    emit('update:roundTimeMinutes', 30) // Shorter rounds for limited
    emit('update:topCut', null) // Usually no top cut
...
watch(() => props.bestOf, (newBestOf) => {
  if (isTournament.value) {
    ...
    emit('update:roundTimeMinutes', newBestOf === 3 ? 50 : 25)
  }
})
</script>
...
<template>
  <div class="space-y-4">
    ...
    <div class="space-y-3">
      ...
      <!-- Limited event guidance -->
      <div v-if="limitedGuidance" class="bg-amber-50 border border-amber-100 rounded-lg p-4">
        <h4 class="text-sm font-medium text-amber-800 mb-2 flex items-center gap-2">
          ...
          {{ limitedGuidance.title }}
        </h4>
        <p class="text-sm text-amber-700 mb-2">{{ limitedGuidance.description }}</p>
        <ul class="text-sm text-amber-700 space-y-1">
          <li v-for="tip in limitedGuidance.tips" :key="tip" class="flex items-start gap-2">
    ...
    <!-- Tournament-specific options (only for competitive events) -->

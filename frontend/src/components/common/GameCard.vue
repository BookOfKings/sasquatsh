      ...
      <div class="flex items-start justify-between gap-2">
        <div class="min-w-0 flex-1">
          ...
          <div class="flex flex-wrap gap-x-2 gap-y-1 mt-1">
            <span v-if="game.minPlayers || game.maxPlayers" class="text-xs text-gray-500 whitespace-nowrap">
            ...
            <span v-if="game.playingTime" class="text-xs text-gray-500 whitespace-nowrap">
              <svg class="w-3 h-3 inline mr-1" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12,20A8,8 0 0,0 20,12A8,8 0 0,0 12,4A8,8 0 0,0 4,12A8,8 0 0,0 12,20M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22C6.47,22 2,17.5 2,12A10,10 0 0,1 12,2M12.5,7V12.25L17,14.92L16.25,16.15L11,13V7H12.5Z"/>
              </svg>
              {{ formatPlaytime(game.playingTime) }}
            </span>
            <span v-if="game.weight" :class="getWeightColor(game.weight)" class="text-xs whitespace-nowrap">
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
          ...
          <span v-if="primary" class="chip-primary text-xs whitespace-nowrap">Primary</span>
          <button
            v-if="removable"
            type="button"

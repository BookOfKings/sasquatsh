...
<template>
  ...
  <div v-else class="container-wide py-8">
    <div class="card p-6">
      ...
      <!-- Only show if: loading, OR has member groups, OR has NO groups at all (prompt to browse) -->
      <div v-if="loadingGroups || memberGroups.length > 0 || (managedGroups.length === 0 && memberGroups.length === 0)" class="mb-6">
        <div class="flex items-center gap-3 mb-4">
          <svg class="w-6 h-6 text-secondary-500" viewBox="0 0 24 24" fill="currentColor">
